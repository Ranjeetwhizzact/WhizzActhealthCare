...
$(document).ready(function () {
    ...
    // Utility to capitalize day name
    function capitalize(str) {
        return str.charAt(0).toUpperCase() + str.slice(1);
    }

    // Add day from the available list (from Add button)
    $('#add-day').click(function () {
        let day = days.find(d => !$('#schedule-fields #' + d).length);
        if (day) {
            addDay(day);
            renderMissingDays();
        } else {
    ...
    // Core function to add a schedule field for a day
    function addDay(day) {
    ...
    // Trigger validation when editing time input
    ...
    // Validate time logic (stub, implement as required)
    function validateTime(start, end, div) {
        if (start >= end) {
            alert("End time must be after start time.");
            div.find('.end-time').val('');
        }
    }

    // Remove a day from schedule fields
    ...
    // Dynamically render buttons to re-add missing days
    function renderMissingDays() {
        $('#missing-days').empty();
        days.forEach(day => {
            if (!$('#schedule-fields #' + day).length) {
                $('#missing-days').append(`
                    <button type="button" class="add-missing-day-btn" data-day="${day}">
                        Add ${capitalize(day)}
                    </button>
                `);
            }
    ...
    // Handle click on missing day re-add buttons
    $('#missing-days').on('click', '.add-missing-day-btn', function () {
        let day = $(this).data('day');
        addDay(day);
        renderMissingDays();
    });

    // On page load, add the first day and render missing days
    addDay(days);
    renderMissingDays();
});
